...
const VoiceRSSWebApi = require('./node_modules//voice-rss/src/voice-rss-api');
let Audio = require('web-audio-api').AudioContext;
const app = express();
let jokeInput;
let audioToSend;
let audio;
...
const VoiceRSS = new VoiceRSSWebApi('d4bb3b3e69504439b7518ca9b6f461c7');
...
sendJoke = (jokeInput) => {
    console.log('from sendjoke ffffffffuuu', jokeInput);
    VoiceRSS.getAudio(jokeInput, {}, (err, audio) => {
        if (err) console.error(err);
        else audioToSend = audio;
        console.log.apply(audioToSend)
    });
}
